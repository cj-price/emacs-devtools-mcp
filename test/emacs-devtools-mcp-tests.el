...
(ert-deftest emacs-devtools-mcp-tests/dispatch-handler-quit-becomes-content-block ()
  "A `quit' raised mid-handler is caught and returned as `isError: t'.
A `:slow' tool wraps its body in `while-no-input', which raises
`quit' when user input arrives mid-handler.  Without an explicit
clause, `quit' would unwind past `tools/call' entirely and the
caller would time out instead of receiving a structured
`cancelled' envelope."
  :tags '(:fast)
  (emacs-devtools-mcp-tests--with-fresh-registry
    (eval
     '(emacs-devtools-mcp-deftool quitter
          "Always quits."
        :handler (lambda (_p) (signal 'quit nil)))
     t)
    (let ((reply (emacs-devtools-mcp-server-default-dispatcher
                  nil 'tools/call
                  '(:name "quitter" :arguments nil))))
      (should (eq t (plist-get reply :isError)))
...
(ert-deftest emacs-devtools-mcp-tests/spawn-tools-registered ()
  ...
  :tags '(:fast)
  (dolist (n '("spawn_emacs" "attach_emacs" "kill_spawn" "list_handles"))
...
(ert-deftest emacs-devtools-mcp-tests/spawn-tools-list-includes-all ()
  ...
  (let ((rec (gethash "attach_emacs" emacs-devtools-mcp--tool-registry)))
    ...
    (should (plist-get rec :destructive))
    (should-not (plist-get rec :idempotent)))
  (let ((rec (gethash "kill_spawn" emacs-devtools-mcp--tool-registry)))
    (should-not (plist-get rec :read-only))
    (should (plist-get rec :destructive))
    (should (plist-get rec :idempotent))))
...
(ert-deftest emacs-devtools-mcp-tests/spawn-tool-kill-idempotent ()
  "Calling `kill_spawn' twice returns success on the second call."
  :tags '(:fast)
  (emacs-devtools-mcp-tests--with-empty-handles
   (let ((now (float-time)))
...
(ert-deftest emacs-devtools-mcp-tests/spawn-tool-schema-rejects-bad-args ()
  "`attach-emacs' schema requires `server_name'; `kill-spawn' requires `handle'."
  :tags '(:fast)
  (should
...
(ert-deftest emacs-devtools-mcp-tests/spawn-tool-kill-unknown-handle-ok ()
  "Calling `kill-spawn' with an unknown handle returns a success envelope.
...
`kill_spawn' call -- or a 30-min idle reaper sweep -- would then
...
prefixes are absent from `:backtrace'."
  ...
  (let* ((res (emacs-devtools-mcp-tools-eval--capture-backtrace
    ...
    ;; The signaling frame is at the top.  Emacs 30.1 surfaces an
    ;; explicit `(signal error ...)' frame above `(error ...)'; 30.2
    ;; collapses it.  Accept either.
    (should (string-match-p "\\`(\\(?:signal +\\)?error " bt))))
...
(ert-deftest emacs-devtools-mcp-tests/buffer-substring-redacts-text ()
  "`buffer-substring' output flows through `emacs-devtools-mcp-redact'.
Otherwise an agent could read raw `*Messages*' content -- including
`auth-source-' / `epg-' / `tramp-' lines that `list-messages' is
careful to scrub -- by asking for the buffer span directly.  The
non-truncated and the byte-capped branches both redact."
  :tags '(:fast)
  (with-temp-buffer
    (rename-buffer "edmcp-substring-redact" t)
    (insert "keep-line-1\nauth-source-leak abc\nkeep-line-2\n")
    (let* ((res (edmcp--tools-buffer-substring
                 `(:buffer "edmcp-substring-redact"
                   :start ,(point-min)
                   :end ,(point-max))))
           (text (plist-get res :text)))
      (should (eq :json-false (plist-get res :truncated)))
      (should (string-match-p "keep-line-1" text))
      (should (string-match-p "keep-line-2" text))
      (should-not (string-match-p "auth-source-leak" text)))
    ;; Same redaction must occur on the truncated branch.
    (let* ((res (edmcp--tools-buffer-substring
                 `(:buffer "edmcp-substring-redact"
                   :start ,(point-min)
                   :end ,(point-max)
...
(ert-deftest emacs-devtools-mcp-tests/keys-json-safe-passthrough ()
  "Non-raw input is returned unchanged (or nil for nil)."
  :tags '(:fast)
  (should (null (emacs-devtools-mcp-tools-keys--json-safe nil)))
  (should (equal "" (emacs-devtools-mcp-tools-keys--json-safe "")))
  (should (equal "abc" (emacs-devtools-mcp-tools-keys--json-safe "abc")))
  ;; Non-raw multibyte chars (real Unicode) pass through untouched.
  (should (equal (string ?\u00e9)
                 (emacs-devtools-mcp-tools-keys--json-safe (string ?\u00e9))))
  (should (equal (string ?\u4e2d)
                 (emacs-devtools-mcp-tools-keys--json-safe (string ?\u4e2d)))))

(ert-deftest emacs-devtools-mcp-tests/keys-json-safe-escapes-raw-bytes ()
  "Chars in the raw-byte range #x3FFF80--#x3FFFFF become `\\xNN' escapes."
  :tags '(:fast)
  ;; Single raw bytes at both ends of the range.
  (should (equal "\\x80"
                 (emacs-devtools-mcp-tools-keys--json-safe (string #x3FFF80))))
  (should (equal "\\xff"
                 (emacs-devtools-mcp-tools-keys--json-safe (string #x3FFFFF))))
  ;; Mixed input: ASCII + raw byte at start, middle, and end.
  (should (equal "\\x80abc"
                 (emacs-devtools-mcp-tools-keys--json-safe
                  (concat (string #x3FFF80) "abc"))))
  (should (equal "ab\\x90cd"
                 (emacs-devtools-mcp-tools-keys--json-safe
                  (concat "ab" (string #x3FFF90) "cd"))))
  (should (equal "abc\\xff"
                 (emacs-devtools-mcp-tools-keys--json-safe
                  (concat "abc" (string #x3FFFFF))))))

(ert-deftest emacs-devtools-mcp-tests/keys-json-safe-range-boundaries ()
  "Boundary chars on either side of the raw-byte range behave correctly.
The upper raw-byte sentinel #x3FFFFF coincides with `max-char', so
the \"just above\" case (#x400000) is not constructible in Emacs --
the lower-bound side and both ends of the escaped range cover the
predicate fully."
  :tags '(:fast)
  ;; Just below the range: not escaped.
  (should (equal (string #x3FFF7F)
                 (emacs-devtools-mcp-tools-keys--json-safe
                  (string #x3FFF7F))))
  ;; Lower bound: escaped.
  (should (equal "\\x80"
                 (emacs-devtools-mcp-tools-keys--json-safe (string #x3FFF80))))
  ;; Upper bound: escaped (== `max-char').
  (should (equal "\\xff"
                 (emacs-devtools-mcp-tools-keys--json-safe (string #x3FFFFF))))
  ;; Sanity: #x3FFFFF is the largest representable char.
  (should (= #x3FFFFF (max-char))))

(ert-deftest emacs-devtools-mcp-tests/keys-json-safe-output-json-encodable ()
  "The helper's output for raw input round-trips through `json-serialize'."
  :tags '(:fast)
  (let* ((raw (concat "ab" (string #x3FFF80) "cd" (string #x3FFFFF) "ef"))
         (safe (emacs-devtools-mcp-tools-keys--json-safe raw))
         (encoded (json-serialize (vector safe)))
         (decoded (json-parse-string encoded :array-type 'list)))
    (should (equal "ab\\x80cd\\xffef" safe))
    (should (equal (list safe) decoded))))
...
(ert-deftest emacs-devtools-mcp-tests/describe-keymap-global-walks-to-completion ()
  "`describe_keymap' of `global-map' paginates to completion without `isError'.
Regression test: `global-map' carries bindings whose key sequence
includes raw 8-bit bytes (terminal escape leftovers).  Their
`key-description' is a multibyte string with chars in the raw-byte
range #x3FFF80--#x3FFFFF, which `json-serialize' rejects with
`wrong-type-argument json-value-p'.  Walking the map through the
`tools/call' dispatcher must reach end-of-stream cleanly --
neither the final page nor any intermediate page may come back as
an `isError' envelope."
  :tags '(:daemon)
  (emacs-devtools-mcp-tests--with-fresh-cursors
    (let ((cursor nil)
          ...
          (max-pages 200))
      (cl-loop
       for env = (edmcp--server-tools-call
                  nil
                  (list :name "describe_keymap"
                        :arguments
                        (if cursor
                            (list :keymap "global-map" :cursor cursor)
                          (list :keymap "global-map"))))
       do
       (cl-incf pages)
       (should (eq :json-false (plist-get env :isError)))
       ;; Each text content block must itself be valid JSON-encodable
       ;; -- the bug surfaced as `json-value-p' rejecting the encoded
       ;; content, so re-encoding the envelope here pins the contract.
       (let ((text (plist-get (aref (plist-get env :content) 0) :text)))
         (should (stringp text))
         (let ((parsed (json-parse-string text :object-type 'plist
                                          :array-type 'list)))
           (cl-incf total (length (plist-get parsed :bindings)))
           (setq cursor (plist-get parsed :next_cursor))))
       while (and cursor (< pages max-pages)))
      (should (null cursor))
      (should (> total 0))
      (should (< pages max-pages)))))
