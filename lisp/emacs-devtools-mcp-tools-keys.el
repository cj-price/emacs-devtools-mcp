...
(defun emacs-devtools-mcp-tools-keys--json-safe (s)
  "Return S with Emacs raw-byte chars escaped to `\\xNN' form.
`key-description' on a key sequence whose event is a raw 8-bit
byte (terminal escape leftovers, etc.) yields a multibyte string
containing chars in the range #x3FFF80--#x3FFFFF.  Those code
points are outside Unicode and `json-serialize' rejects them
with `wrong-type-argument json-value-p'.  Replacing each raw
byte with its `\\xNN' escape preserves the binding (no entry is
dropped) and produces a string `json-serialize' accepts."
  (when (stringp s)
    (mapconcat (lambda (ch)
                 (if (and (>= ch #x3FFF80) (<= ch #x3FFFFF))
                     (format "\\x%02x" (- ch #x3FFF00))
                   (string ch)))
               s "")))
                   ...
                   (lambda (event binding)
                     (let* ((seq (vconcat prefix (vector event))))
                       (cond
                        ...
                        (t
                         (push
                          (cons (emacs-devtools-mcp-tools-keys--json-safe
                                 (key-description seq))
