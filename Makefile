...
#
# Targets:
#   all          lisp + test + lint + manual (the CI gate)
...
#   manual       makeinfo manual/emacs-devtools-mcp.texi
...
TEXI_SRC     = $(wildcard $(MANUAL_DIR)/*.texi)
INFO_OUT     = $(TEXI_SRC:.texi=.info)
...
.PHONY: all lisp test test-fast test-daemon test-gui test-mcp lint manual clean install

all: lisp test lint manual
...
$(MANUAL_DIR)/%.info: $(MANUAL_DIR)/%.texi
	makeinfo --no-split -o $@ $<

clean:
	rm -f $(LISP_ELC) $(INFO_OUT)
	rm -f $(MANUAL_DIR)/dir
	find . -name '*~' -delete
